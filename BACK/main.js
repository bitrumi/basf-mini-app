...
function showBrand(brand) {
  ...
  brandTables.forEach((table) => {
    ...
    if (isTarget) {
      // лесенка для строк таблицы
      table.classList.remove('show-animated');
      void table.offsetWidth; // форсим reflow
      table.classList.add('show-animated');

      // анимация wow-блоков только для TDK
      if (brand === 'tdk') {
        const wowBlocks = [
        ...
        wowBlocks.forEach((el, i) => {
          ...
          void el.offsetWidth; // сброс transition
          el.style.transitionDelay = `${0.05 * i}s`; // чуть-чуть лесенки
...
// Функции для заполнения wow-блоков (тоже объявлены заранее)
function fillWowBlockForSAX() {
  ...
  const wowBlock = document.getElementById('tape-wow-sa-x');
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  i18next.init(
    ...
    () => {
      updateContent(); // первая отрисовка переводов
    }
  );
  ...
  function openRarityModal() {
    if (rarityModal) {
      ...
      showBrand('sony'); // showBrand уже доступна
    }
  }
  ...
  if (btnCryptomus || modalBtnCryptomus) {
    const attachCryptomusHandler = (buttonEl) => {
      ...
      buttonEl.addEventListener('click', async () => {
        ...
        const res = await fetch('http://localhost:3000/api/cryptomus/create-payment', {
          method: 'POST',
  ...
  if (btnCryptoBot || modalBtnCryptoBot) {
    const attachCryptoBotHandler = (buttonEl, urlTargetEl) => {
      ...
      buttonEl.addEventListener('click', async () => {
        ...
        const res = await fetch('http://localhost:3000/api/cryptobot/create-invoice', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId }),
        });
  ...
  menuItems.forEach((item) => {
    item.addEventListener('click', () => {
      ...
      if (section === 'assessment') {
        ...
        openRarityModal(); // showBrand вызывается внутри openRarityModal
